...
1. WHEN tRPCアーキテクチャドキュメントを作成する THEN システムは `docs/architecture/trpc-architecture.md` にディレクトリ構造、命名規則、設計原則を記載する
2. WHEN マイグレーションガイドを作成する THEN システムは `docs/legacy/trpc-migration.md` に既存API Routesからの移行手順を段階的に記載する
