...
import { roundMarketCaps } from "@/utils/round";
import { describe, expect, it } from "bun:test";

describe("2.2 丸めとマップ構築 + ハッシュ影響検証", () => {
  it("rounds all values to 6 decimals", () => {
    const rounded = roundMarketCaps({ AI: 1.2345678, OIL: 2.9999994 });
  ...
  it("differences beyond 6th decimal produce identical rounded map and identical hash", () => {
    ...
    const ra = roundMarketCaps(rawA);
    const rb = roundMarketCaps(rawB);
