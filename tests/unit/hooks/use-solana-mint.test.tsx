...
describe("unit/hooks/use-solana-mint", () => {
  ...
  it("runs a mint preparation callback with the actual token id for each attempt", async () => {
    const globalConfig = deriveGlobalConfigPda();
    const collection = createPublicKey(21);
    const collectionUpdateAuthority = PublicKey.findProgramAddressSync(
      [textEncoder.encode("collection_authority"), globalConfig.toBytes()],
    ...
    const tokenIds = [INITIAL_TOKEN_ID, INITIAL_TOKEN_ID, RETRY_TOKEN_ID];
    const prepareMintAssetMock = mock(async (_tokenId: bigint) => {
      await Promise.resolve();
    });
    ...
    await act(async () => {
      await result.current.mint({
        prepareMetadata: prepareMintAssetMock,
      });
    });

    expect(prepareMintAssetMock.mock.calls.map(([tokenId]) => tokenId)).toEqual([INITIAL_TOKEN_ID, RETRY_TOKEN_ID]);
  });
});
